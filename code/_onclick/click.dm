...
/mob/proc/ClickOn(atom/target_atom, params)
	...
	if(isturf(target_atom) || isturf(target_atom.loc) || (sdepth != -1 && sdepth <= 1))
		if(target_atom.Adjacent(src) || (wielded_item && wielded_item.attack_can_reach(src, target_atom, wielded_item.reach)) || (simple_mob && simple_mob.attack_can_reach(src, target_atom, simple_mob.melee_reach))) // see adjacent.dm
			if(wielded_item)
